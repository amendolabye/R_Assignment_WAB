...
Plotting position of SNPs on X axis for both maize and teosinte, no difference can be seen across each chromosome in each species - no difference in SNP distribution

```{r}
ggplot(data=maize_snps, mapping=aes(x=Position, y=as.numeric(Chromosome))) + geom_point() + facet_wrap(~as.numeric(Chromosome), scales="free_y") + theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
ggplot(data=teosinte_snps, mapping=aes(x=Position, y=as.numeric(Chromosome))) + geom_point() + facet_wrap(~as.numeric(Chromosome), scales="free_y") + theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
```
